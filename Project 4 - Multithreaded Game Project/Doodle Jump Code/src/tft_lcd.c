...
void TFT_Set_Backlight_Brightness(uint16_t brightness_percent){
	Set_PWM_Value(TFT_BL_TPM, TFT_BL_TPM_CHANNEL, (brightness_percent*100)/TFT_BL_PERIOD);
}
...
void TFT_Init()
{
	...
	TFT_24S_Write_Data(0x0008); // Don't mirror Y axis
...
/** Prepare LCD controller draw rectangle using upcoming pixels. Returns number of pixels expected. 
*/
...
void TFT_TS_Init(void) {
	...
	// Configure GPIO? Done in Read function
}
...
uint32_t TFT_TS_Read(PT_T * position) {
	// Return 1 if valid touch, update position. Else return 0
	uint32_t x, y;
	uint32_t b;
