...
/*********************************************************************************************/
/* Functon name : fei_max(void);
 *
 * Mode transition: Move from FEI to FEI at max clock
 *
 *  Set's trim to 390625 and changes DMX to multiply clock to 100 MHZ
 * Parameters: irc_select  - which internal reference to use
 *             irc_freq    - selects the frequency to use for irc
 *             mcg_out_freq   - selects the desired output frequency of mcg

********************************************************************************/
int fei_maxclk(unsigned char irc_select, int irc_freq, int mcg_out_freq){
...
orig_div = SIM_CLKDIV1; //store present clock divider values
  ...
  MCG_C4 |= MCG_C4_FCTRIM(15) | MCG_C4_SCFTRIM_MASK;  //max trim for 390625
  MCG_C4 |= MCG_C4_DRST_DRS(3);  //set mcg_clk to 100,000,000

  // Check resulting FLL frequency
  mcg_out_freq = fll_freq(irc_freq);
  return mcg_out_freq;
}
...
unsigned char atc(unsigned char irc_select, int irc_freq, int mcg_out_freq)
{
  ...
  if(!irc_select) //determine if slow or fast IRC to be trimmed
  {
    ...
    if (irc_freq > 39063) // check frequency is below max spec.
    {
      SIM_CLKDIV1 = orig_div; // set SIM_CLKDIV1 back to original value
      return 5;
    }
